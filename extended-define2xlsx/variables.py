...
class Variables:
    HEADERS = ["OID", "Order", "Dataset", "Variable", "Label", "Data Type", "Length", "Significant Digits", "Format",
               ...
               "Predecessor", "Role", "Comment", "IsNonStandard", "HasNoData", "Core", "DatePublished", "Status",
               "SubmissionDataType"]
            ...
            for ig in self.mdv.ItemGroupDef:
                for ir in ig.ItemRef:
                    ...
                    writer.writerow([idd["OID"], ird["Order"], ig.Name, idd["Variable"], idd["Label"], idd["Data Type"], idd["Length"],
                                     ...
                                     ird["IsNonStandard"], ird["HasNoData"], idd["Core"], idd["DatePublished"],
                                     idd["Status"], idd["SubmissionDataType"]])
    ...
    def _load_item_def(self, item_oid):
        ...
        idd["Core"] = it.Core if it.Core else ""
        idd["DatePublished"] = it.DatePublished if it.DatePublished else ""
        idd["Status"] = it.Status if it.Status else ""
        idd["SubmissionDataType"] = it.SubmissionDataType if it.SubmissionDataType else ""
        return idd
